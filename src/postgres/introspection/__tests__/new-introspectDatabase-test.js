...
test('introspects greenplum', async () => {
  const pool = new Pool({
    user: 'devuser',
    host: 'localhost',
    database: 'devdb',
  ...
  const catalog = await introspectDatabase(pool, ['dp','dtcm'])

  const formattedCatalog = format(catalog)
  expect(formattedCatalog).toMatchSnapshot()
})
